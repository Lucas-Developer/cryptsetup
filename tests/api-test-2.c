...
static int _setup(void)
{
	...
	_system("dd if=/dev/zero of=" IMAGE_EMPTY " bs=1M count=5 2>/dev/null", 1);
...
#define LUKS2_KEYRING_JSON(x,y) "{\"type\":\"luks2-keyring\",\"keyslots\":[" y "]," \
...
static void LuksConvert(void)
{
	...
	OK_(_system("dd if=" CONV_DIR "/" CONV_L1_256_LEGACY " of=" DMDIR L_DEVICE_1S " bs=1M count=2 oflag=direct", 1));
	OK_(crypt_init(&cd, DMDIR L_DEVICE_1S));
	 ...
	 * with cryptsetup)
	 */
	OK_(_system("dd if=" CONV_DIR "/" CONV_L1_256_UNMOVABLE " of=" DMDIR L_DEVICE_1S " bs=1M count=2 oflag=direct", 1));
	OK_(crypt_init(&cd, DMDIR L_DEVICE_1S));
	OK_(crypt_load(cd, CRYPT_LUKS1, NULL));
	FAIL_(crypt_convert(cd, CRYPT_LUKS2, NULL), "Can't convert device with unaligned keyslot offset");
	crypt_free(cd);
	...
	// 128b key
	OK_(_system("dd if=" CONV_DIR "/" CONV_L1_128 " of=" DMDIR L_DEVICE_1S " bs=1M count=2 oflag=direct", 1));

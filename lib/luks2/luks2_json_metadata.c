		     ...
		     const char *backup_file)
{
	...
	/* FIXME: Do NOT allow overwrite LUKS1 header if it's there */
	...
	if (r == 0) {
		...
		if (buffer_size != (size_t) LUKS2_hdr_and_areas_size(tmp_hdr.jobj)) {
...
static void hdr_dump_config(struct crypt_device *cd, json_object *hdr_jobj)
{
	...
	if (json_object_object_get_ex(hdr_jobj, "config", &jobj_config) &&
	    json_object_object_get_ex(jobj_config, "flags", &jobj_flags))
		for (i = 0; i < json_object_array_length(jobj_flags); i++) {
			jobj1 = json_object_array_get_idx(jobj_flags, i);

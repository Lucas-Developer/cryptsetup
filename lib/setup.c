			       ...
			       struct crypt_params_luks2 *params)
{
	...
	char *_cipher, *_mode;
	const char *integrity = (params && params->integrity) ? params->integrity->integrity : NULL;
	...
	if (integrity) {
		...
		if (params->integrity->journal_crypt || params->integrity->journal_integrity)
			return -ENOTSUP;
	}
	...
	/* Save cipher and mode, compatibility only. */
	_cipher = strdup(cipher);
	_mode = strdup(cipher_mode);
	if (!_cipher || !_mode) {
		r = -ENOMEM;
		goto out;
	}
	...
	if (crypt_get_integrity_tag_size(cd)) {
		...
		r = INTEGRITY_format(cd, params ? params->integrity : NULL, NULL, NULL);
		if (r)
...
out:
	if (r) {
		...
		cd->u.luks2.cipher = _cipher;
		cd->u.luks2.cipher_mode = _mode;
	}
...
{
	return onlyLUKS2(cd) ?: LUKS2_hdr_labels(cd, &cd->u.luks2.hdr, label, subsystem, 1);
}
			 ...
			 const char *backup_file)
{
	...
	if (!cd || (cd->type && !isLUKS1(cd->type) && !isLUKS2(cd->type)))
		return -EINVAL;
	...
	uint32_t flags)
{
	...
	if (isPLAIN(cd->type)) {
		...
		r = LUKS2_keyslot_open(cd, keyslot, name ? 0 : CRYPT_ANY_SEGMENT, passphrase,
				       passphrase_size, &vk);
		if (r >= 0) {
			keyslot = r;
	...
	struct crypt_params_integrity *ip)
{
	...
	if (isINTEGRITY(cd->type)) {
		...
		ip->integrity = LUKS2_get_integrity(&cd->u.luks2.hdr, 0);
		ip->integrity_key_size = crypt_get_integrity_key_size(cd);
